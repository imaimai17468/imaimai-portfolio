...
export const ProgressBar: React.FC<ProgressBarProps> = ({
	...
	const isShowProgressCircle = useMemo(
		() => topRef && aboutRef && skillsRef,
		[topRef, aboutRef, skillsRef],
	);
	...
	return (
		<div>
			<motion.div
				className="fixed bottom-0 left-5 top-0 w-1 origin-top bg-emerald-400"
				style={{ scaleY }}
			/>
			<motion.div
				className="fixed bottom-0 left-4 top-0 w-0.5 bg-emerald-500"
				style={{ scaleY }}
			/>
			<motion.div
			...
			{isShowProgressCircle && (
				<div className="fixed left-10 -z-10 my-10 flex flex-col gap-10 text-2xl font-thin text-gray-200 lg:z-10">
					...
					>
						...
						<span className="hidden transition-all hover:text-emerald-400 lg:block">
							Top
						</span>
					</a>
					<a
